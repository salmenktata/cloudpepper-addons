...
class StockPicking(models.Model):
    ...
    def _quelyos_apply_auto_source_strategy(self):
        ...
        basis = _get_param("stock_basis", "free")  # free | onhand | forecast
        ...
        # Création des réassorts internes
        if replenish_data:
        ...
        # Réservation automatique du recordset
        try:
            self.action_assign()
    ...
    def _get_available_quantities(self, product_ids, location_ids, basis):
        ...
        if basis == "free":
            ...
            for q in quants:
                if 'product_id' in q and 'location_id' in q:
                    loc_id = q['location_id'][0]
                    prod_id = q['product_id'][0]
                    available_qty = q.get('quantity', 0.0) - q.get('reserved_quantity', 0.0)
                    res[loc_id][prod_id] = max(0.0, available_qty)
        else:
            # onhand | forecast (moins perf, acceptable si liste courte)
            for loc_id in location_ids:
                if not loc_id:
                    continue
                loc = self.env['stock.location'].browse(loc_id).with_context(compute_child=True)
                for product in self.env['product.product'].browse(product_ids):
                    prod_ctx = product.with_context(location=loc.id)
                    if basis == "onhand":
