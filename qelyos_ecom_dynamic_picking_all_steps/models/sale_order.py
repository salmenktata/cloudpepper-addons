...
class SaleOrder(models.Model):
    ...
    def _warehouse_key_locs(self, picking):
        ...
        if not wh:
            return {}
        return {
            'stock': wh.lot_stock_id,  # Stock central
    ...
    def _classify_delivery_step(self, picking):
        # Returns 'one_step_out', 'two_step_pick', 'three_step_pick', or 'outgoing'
        # based on source/destination vs warehouse key locations.
    ...
    def _choose_location_for_picking(self, company, picking):
        ...
        # Strategy chain CENTRAL -> GST max -> fixed order
    ...
    def _action_confirm(self):
        ...
        for order in self:
            ...
            # Determine which leg to alter based on 1/2/3 step delivery
            # Prefer: three_step_pick > two_step_pick > outgoing
            target = None
            for p in pickings:
            ...
            # Apply to one recordset (either single browse record or length-1 recordset)
            for picking in targets:
                ...
                self._apply_source_location(picking, chosen_loc, reason + " | Flux géré 1/2/3 étapes.")
        return res
