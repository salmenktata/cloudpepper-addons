...
class SaleOrder(models.Model):
    ...
    def _get_available_qty(self, product, location):
        # Odoo available qty (excludes reservations), for a location (includes children)
    ...
    def _score_location_for_picking(self, picking, loc):
        if not picking.move_ids_without_package:
            return 0.0
        # Score is min available across lines (guarantees single-source coverage if >= required per line)
    ...
    def _warehouse_key_locs(self, picking):
        ...
        if not wh:
            return {}
        return {
            'stock': wh.lot_stock_id,  # Central Stock
    ...
    def _choose_location_for_picking(self, company, picking):
        ...
        # Identify CENTRAL from warehouse and ensure it's considered
        ...
        if strategy == 'central_then_maxstock_then_order':
            # Step 1: CENTRAL using AVAILABLE qty (excludes reservations)
            if central_loc:
            ...
            # Step 2: Max available among Gafsa/Sousse/Tunis
            ...
            if gst_locations:
                ...
                if scored:
                    ...
                    if best[1] > 0:
                        return best[0], f"Critère 2: Max stock (disponible) Gafsa/Sousse/Tunis → {best[0].display_name} (score={best[1]})"
    ...
    def _action_confirm(self):
        ...
        for order in self:
            ...
            # Choose the leg to modify: 3-step pick > 2-step pick > 1-step outgoing
            target = None
            for p in pickings:
