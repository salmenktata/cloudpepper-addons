...
class SaleOrder(models.Model):
    ...
    def _action_confirm(self):
        # After routes applied by super(), we force source for ecommerce orders
        res = super()._action_confirm()
        for order in self:
            company = order.company_id
            website_limit = company.ecom_dynamic_website_id
            # Only for ecommerce orders (and optional website filter)
            if not order.website_id or (website_limit and order.website_id != website_limit):
                continue

            # Candidate locations: only INTERNAL
            ...
            for picking in pickings:
                    ...
                    if not chosen_loc:
                        ...
                        if gst_locations:
                            ...
                            if scored:
                                ...
                                if best[1] > 0:
                                    chosen_loc = best[0]
                                    reason = f"Critère 2: Max stock parmi Gafsa/Sousse/Tunis → {chosen_loc.display_name} (score={best[1]})"
                    # 3) sinon ordre fixe
                    if not chosen_loc:
                ...
                # Force source even if routes created moves elsewhere
                if chosen_loc:
                    # If chosen location does not fully cover, let your other module handle internal transfer if configured
                    self._apply_source_location(picking, chosen_loc, reason + " | Forçage après application des routes (2 étapes).")
